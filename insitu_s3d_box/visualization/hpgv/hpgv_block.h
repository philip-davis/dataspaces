...
typedef struct blk_header_t {
    ...
    float *blk_coord_obj2grid[3];
    float *blk_coord_grid2obj[3];
...
typedef struct volume_data_t {
    ...
    int             data_original_elenum;
    void            *data_original;    
    
    int             data_boundary_x_elenum[2];
    void            *data_boundary_x[2];    
    
    int             data_boundary_y_elenum[2];
    void            *data_boundary_y[2];
    
    int             data_boundary_z_elenum[2];
...
typedef struct block_t {
    ...
    /* section for intransit processing */
    float               depth;
    int                 visorder;
    float               global_min;
    float               global_max;
    int                 org_domain_grid_size[3];
    int                 sample_step;
} block_t;
...
uint8_t *block_serialize(block_t *block, int *outsize);

block_t *block_deserialzie(uint8_t *buffer);

void block_free(block_t *block, int freebuf);
