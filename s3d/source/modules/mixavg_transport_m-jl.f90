...
!  But remember that both refer to the same memory location
    ...
    real, intent(inout),  target, dimension(nx,ny,nz,3,n_spec) :: grad_Ys
    ...
    ! compute diffusive flux for species n in direction m.
      !call pat_region_begin(7,'computespecies',ierr)
    diffFlux(:,:,:,:,n_spec) = 0.0
!   DIRECTION: do m=1,3
          if (baro_switch.and.thermDiff_switch) then
        do n=1,n_spec-1
        do k=1,nz
        do j=1,ny
        do i=1,nx
    ...
    do k = 1, nz
      do j = 1, ny
         do i = 1, nx
           ...
           grad_u_tmp(1,1) = grad_u(i,j,k,1,1)
           grad_u_tmp(2,1) = grad_u(i,j,k,2,1)
           grad_u_tmp(3,1) = grad_u(i,j,k,3,1)
           grad_u_tmp(1,2) = grad_u(i,j,k,1,2)
           grad_u_tmp(2,2) = grad_u(i,j,k,2,2)
           grad_u_tmp(3,2) = grad_u(i,j,k,3,2)
           grad_u_tmp(1,3) = grad_u(i,j,k,1,3)
           grad_u_tmp(2,3) = grad_u(i,j,k,2,3)
           grad_u_tmp(3,3) = grad_u(i,j,k,3,3)

!          do n=1,3
              sumterm = grad_u_tmp(1,1)+grad_u_tmp(2,2)+grad_u_tmp(3,3)
!          enddo

!          ROW: do m=1,3
              tau(i,j,k,1,1) = 2.0*viscosity(i,j,k) * (grad_u_tmp(1,1) - sumterm/3.0)
!             COLUMN: do n=m+1,3
                 tau(i,j,k,2,2) = viscosity(i,j,k) * ( grad_u_tmp(1,2) + grad_u_tmp(2,1) )
                 tau(i,j,k,2,1) = tau(i,j,k,1,2)
                 tau(i,j,k,3,3) = viscosity(i,j,k) * ( grad_u_tmp(1,3) + grad_u_tmp(3,1) )
                 tau(i,j,k,3,1) = tau(i,j,k,1,3)
              tau(i,j,k,2,2) = 2.0*viscosity(i,j,k) * (grad_u_tmp(2,2) - sumterm/3.0)
                 tau(i,j,k,3,3) = viscosity(i,j,k) * ( grad_u_tmp(1,3) + grad_u_tmp(3,1) )
                 tau(i,j,k,3,1) = tau(i,j,k,1,3)
              tau(i,j,k,3,3) = 2.0*viscosity(i,j,k) * (grad_u_tmp(3,3) - sumterm/3.0)
!             enddo COLUMN
!          enddo ROW
        end do !i
      end do ! j
...
!----------------------------------------------------------------------
    ...
    real, intent(in),  dimension(nx,ny,nz,3,n_spec) :: diffFlux
...
!   DIRECTION: do m=1,3
          ...
          do j=1,ny
          do i=1,nx
       heatFlux(i,j,k,1) = -lambda(i,j,k) * grad_T(i,j,k,1)
       heatFlux(i,j,k,2) = -lambda(i,j,k) * grad_T(i,j,k,2)
       heatFlux(i,j,k,3) = -lambda(i,j,k) * grad_T(i,j,k,3)
          enddo
          enddo
          enddo
     ...
     ! add in the mass diffusion component.
       SPECIES: do n=1,n_spec
          ...
          do j=1,ny
          do i=1,nx
          heatFlux(i,j,k,1) = heatFlux(i,j,k,1) + enthalpy(i,j,k,n)*diffFlux(i,j,k,1,n)
          heatFlux(i,j,k,2) = heatFlux(i,j,k,2) + enthalpy(i,j,k,n)*diffFlux(i,j,k,2,n)
          heatFlux(i,j,k,3) = heatFlux(i,j,k,3) + enthalpy(i,j,k,n)*diffFlux(i,j,k,3,n)
          enddo
          enddo
          enddo
       enddo SPECIES
...
subroutine getDiffusiveFluxTerms( grad_u, Temp, grad_T, Ys, grad_Ys, Press, rho, & 
  ...
  real, intent(inout), dimension(nx,ny,nz,3,n_spec)  :: grad_Ys
