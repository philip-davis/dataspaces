  ...
  subroutine allocate_bc_arrays(flag)
    ...
    if(flag.eq.1) then
       ...
       if(nrf_x0<=0 .or. nrf_xl<=0) then
          allocate(qx_bc(2,ny,nz,nvar_tot+1))
          qx_bc=0.0
       endif

       if(nrf_y0<=0 .or. nrf_yl<=0) then
          allocate(qy_bc(nx,2,nz,nvar_tot+1))
          qy_bc=0.0
       endif

       if(nrf_z0<=0 .or. nrf_zl<=0) then
          allocate(qz_bc(nx,ny,2,nvar_tot+1))
          qz_bc=0.0
       endif
...
!=========================================================================================
  ...
  ! L = 5 -> temperature
    ...
    if(nrf_x0<=0 .or. nrf_xl<=0) then
       ...
       qx_bc(1,:,:,5) = temp( 1,:,:)
       qx_bc(2,:,:,5) = temp(nx,:,:)
       qx_bc(:,:,:,nvar_tot+1) = qx_bc(:,:,:,4)-sum(qx_bc(:,:,:,6:nvar_tot))
    endif
    ...
    if(nrf_y0<=0 .or. nrf_yl<=0) then
       ...
       qy_bc(:,1,:,5) = temp(:, 1,:)
       qy_bc(:,2,:,5) = temp(:,ny,:)
       qy_bc(:,:,:,nvar_tot+1) = qy_bc(:,:,:,4)-sum(qy_bc(:,:,:,6:nvar_tot))
    endif
    ...
    if(nrf_z0<=0 .or. nrf_zl<=0) then
       ...
       qz_bc(:,:,1,5) = temp(:,:, 1)
       qz_bc(:,:,2,5) = temp(:,:,nz)
       qz_bc(:,:,:,nvar_tot+1) = qz_bc(:,:,:,4)-sum(qz_bc(:,:,:,6:nvar_tot))
    endif
