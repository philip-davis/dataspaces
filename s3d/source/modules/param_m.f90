...
! integers
  ...
  integer iselect               !parameter to determine post processing options
...
!   check for domain size conflict with stencils size in x-direction
    ...
    if((vary_in_x.eq.1).and.(nx.lt.9)) then

      if(myid.eq.0) then
        write(io,*) 'input error: nx < 9 and vary_in_x = 1'
      endif
...
!   check for domain size conflict with stencils size in y-direction
    ...
    if((vary_in_y.eq.1).and.(ny.lt.9)) then

      if(myid.eq.0) then
        write(io,*) 'input error: ny < 9 and vary_in_y = 1'
      endif
...
!   check for domain size conflict with stencils size in z-direction
    ...
    if((vary_in_z.eq.1).and.(nz.lt.9)) then

      if(myid.eq.0) then
        write(io,*) 'input error: nz < 9 and vary_in_z = 1'
      endif
