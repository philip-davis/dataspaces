...
! module for Read-Write Restart files using MPI I/O
    ...
    integer file_info, threeD_ftype
...
subroutine mpi_io_set_filetype_interp(nx,ny,nz,nx_g,ny_g,nz_g,flag)
    use topology_m,  only : npes, mypx, mypy, mypz
!    use param_m,     only : nx, ny, nz, nx_g, ny_g, nz_g
    implicit none
    include 'mpif.h'
    ...
    ! declarations passed in
    integer, intent(in) :: nx,ny,nz,nx_g,ny_g,nz_g
    integer, intent(in) :: flag ! 0 for creating data type, -1 for freeing it up
    ...
    return
end subroutine mpi_io_set_filetype_interp

!----< mpi_file_io() >----version for interpolation----------------------------
subroutine mpi_file_int_io(yspecies,u,pressure,temp,nx,ny,nz,filename) 
!subroutine mpi_file_io(filename,rw)
    use topology_m,  only : gcomm
!    use param_m,     only : nx, ny, nz, nsc
...
!    use variables_m, only : temp, pressure, yspecies, u
    ...
    real, dimension(nx,ny,nz,nsc+1), intent(out) :: yspecies
    real, dimension(nx,ny,nz,3), intent(out) :: u
    real, dimension(nx,ny,nz), intent(out) :: pressure
...
!    endif
    ...
    return
end subroutine mpi_file_int_io
