subroutine rndini2d_mix( seed, ke, xim, xir, io, mixfr)
!
...
!gbansal/04/18/08: Correction as per Evatt's email:
	      e_spec = c*e_spec
...
!----------------------------------------------------------------------------------------
!     step 2. compute the non-normalized mixture fraction rms and 
...
!     xirs = sqrt(xirs/real(nx*ny))
  xirs = sqrt(xirs_sum / real(nx_g * ny_g))
  xirs = xir / xirs
...
!-------- a better way to impose the limits on xi should be implemented --------
  do i = 1, nx
     do j = 1, ny
        mixfr(i,j) = xim + xirs*mixfr(i,j)
        mixfr(i,j) = min(0.9999999999,mixfr(i,j))
        mixfr(i,j) = max(0.0000000001,mixfr(i,j))
