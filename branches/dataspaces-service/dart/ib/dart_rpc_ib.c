...
static int sys_send (struct rpc_server *rpc_s, struct node_id *to,
		     struct hdr_sys *hs);
/* =====================================================
	Fabric:
	(1)barrier functions
...
static int
sys_bar_arrive (struct rpc_server *rpc_s, struct hdr_sys *hs)	//Done
{
...
struct node_id *
rpc_server_find (struct rpc_server *rpc_s, int nodeid)
{
  struct node_id *temp_peer;
  list_for_each_entry (temp_peer, &rpc_s->peer_list, struct node_id,
		       peer_entry)
  {
...
static int
sys_bar_send (struct rpc_server *rpc_s, int peerid)	//Done
{
...
*/
static int
sys_send (struct rpc_server *rpc_s, struct node_id *peer, struct hdr_sys *hs)	//Done
{
...
//              goto err_out;
  ...
  sm->hs = *hs;
  sm->sys_mr =
    ibv_reg_mr (peer->sys_conn.pd, &sm->hs, sizeof (struct hdr_sys),
...
static int
sys_dispatch_event (struct rpc_server *rpc_s, struct hdr_sys *hs)	//Done
{
  int err = 0;
...
static int
sys_process_event (struct rpc_server *rpc_s)
{
  struct node_id *peer;
...
//                              peer = rpc_s->peer_tab + which_peer[i];
	      ...
	      err =
		ibv_get_cq_event (peer->sys_conn.comp_channel, &ev_cq,
				  &ev_ctx);
	      ...
	      do
		{
		  ...
		  if (wc.opcode & IBV_WC_RECV)
		    {
		      struct hdr_sys *hs =
			(struct hdr_sys *) (uintptr_t) wc.wr_id;
...
static int
sys_cleanup (struct rpc_server *rpc_s)
{
  int i, err = 0;
  struct node_id *peer;
...
//              peer = &rpc_s->peer_tab[i];
      if (peer->sys_conn.f_connected == 0
	  || peer->ptlmap.id == rpc_s->ptlmap.id)
	continue;
...
static struct node_id *
rpc_get_peer (struct rpc_server *rpc_s, int peer_id)
{
...
static int
rpc_cb_cleanup (struct rpc_server *rpc_s, struct rpc_cmd *cmd)
{
...
static int
rpc_cb_decode (struct rpc_server *rpc_s, struct ibv_wc *wc)	//Done
{
...
//      }
  ...
  if (err < 0)
    printf ("(%s) err: Peer# %d Network command %d from %d.\n", __func__,
	    rpc_s->ptlmap.id, cmd->cmd, cmd->id);
...
rr_comm_alloc (int num_rpc)	//Done
{
  struct rpc_request *rr;
  size_t size;

  size =
    sizeof (*rr) + sizeof (*rr->msg) + sizeof (struct rpc_cmd) * num_rpc + 7;
...
*/
static int
rpc_cb_req_completion (struct rpc_server *rpc_s, struct ibv_wc *wc)	//Working
{
  int err;
  struct rpc_request *rr = (struct rpc_request *) (uintptr_t) wc->wr_id;
  if (!rr)
...
*/
static int
rpc_prepare_buffers (struct rpc_server *rpc_s, const struct node_id *peer, struct rpc_request *rr, enum io_dir dir)	//working: TODO need 'id' information getting from initialization
{
  int err;

  rr->msg->msg_rpc->wr_id = (uintptr_t) rr;
  rr->data_mr =
    ibv_reg_mr (peer->rpc_conn.pd, rr->msg->msg_data, rr->msg->size,
		IBV_ACCESS_LOCAL_WRITE | ((dir == io_send) ?
...
static int
rpc_post_request (struct rpc_server *rpc_s, const struct node_id *peer, struct rpc_request *rr, const struct hdr_sys *hs)	//working 
{
  int err;
  ...
  if (rr->type == 0)
    {
      rr->rpc_mr =
	ibv_reg_mr (peer->rpc_conn.pd, rr->data, rr->size,
		    IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_READ);
      if (rr->rpc_mr == NULL)
	{
	  printf ("ibv_reg_mr return NULL.\n");
  ...
  if (err < 0)
    {
      ...
      printf ("Node %d has %d send_event on the queue (max %d) to peer %d.\n",
	      rpc_s->ptlmap.id, peer->req_posted,
	      peer->rpc_conn.qp_attr.cap.max_send_wr, peer->ptlmap.id);
      goto err_out;
    }
...
static int
rpc_fetch_request (struct rpc_server *rpc_s, const struct node_id *peer, struct rpc_request *rr)	//TODO id
{
  int err;
  struct ibv_send_wr wr, *bad_wr = NULL;
  struct ibv_sge sge;
...
static int
peer_process_send_list (struct rpc_server *rpc_s, struct node_id *peer)	// working
{
  struct rpc_request *rr;
//      struct hdr_sys hs;
  int err, i;
...
int
register_memory2 (struct rpc_request *rr, struct node_id *peer)
{
  rr->rpc_mr =
    ibv_reg_mr (peer->rpc_conn.pd, rr->msg->msg_data, rr->msg->size,
		IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
...
struct rpc_request *
register_memory (struct node_id *peer)
{
...
int
post_receives (struct rpc_request *rr, struct node_id *peer, int i)
{
...
*/
int
rpc_barrier (struct rpc_server *rpc_s)	//Donesys_bar_send
{
  //      struct node_id *peer;
  int round, np;
  int next, prev;
  ...
  while (round < np - 1)
    {
      ...
      prev =
	(rpc_s->app_num_peers + myrank (rpc_s) -
	 (1 << round)) % rpc_s->app_num_peers;
...
struct rpc_server *
rpc_server_init (int option, char *ip, int port, int num_buff,
		 int num_rpc_per_buff, void *dart_ref,
		 enum rpc_component cmp_type)
{
  ...
  err =
    rdma_bind_addr (rpc_s->listen_id,
		    (struct sockaddr *) &rpc_s->ptlmap.address);
  if (err != 0)
    {
...
int
rpc_post_recv (struct rpc_server *rpc_s, struct node_id *peer)
{
  int err;
...
int
sys_post_recv (struct rpc_server *rpc_s, struct node_id *peer)
{
  ...
  sm->sys_mr =
    ibv_reg_mr (peer->sys_conn.pd, hs,
		rpc_s->num_buf * sizeof (struct hdr_sys),
		IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE);
  if (sm->sys_mr == NULL)
    {
      printf ("ibv_reg_mr return NULL.\n");
...
static int
rpc_server_finish (struct rpc_server *rpc_s)
{
  struct node_id *peer;
  int i, err;
...
//working on this now
int
rpc_server_free (struct rpc_server *rpc_s)
{
  int err, i;
...
*/
  ...
  list_for_each_entry_safe (rr, tmp, &rpc_s->rpc_list, struct rpc_request,
			    req_entry)
  {
    err = ibv_dereg_mr (rr->rpc_mr);
    if (err != 0)
...
void
rpc_server_set_peer_ref (struct rpc_server *rpc_s, struct node_id peer_tab[], int num_peers)	//Done
{
...
void
rpc_server_set_rpc_per_buff (struct rpc_server *rpc_s, int num_rpc_per_buff)	//Done
{
  rpc_s->num_rpc_per_buff = num_rpc_per_buff;
}

struct rpc_server *
rpc_server_get_instance (void)	//Done
{
...
*/
void
build_context (struct ibv_context *verbs, struct connection *conn)
{
  int err, flags;
  conn->ctx = verbs;
...
void
build_qp_attr (struct ibv_qp_init_attr *qp_attr, struct connection *conn,
...
int
rpc_connect (struct rpc_server *rpc_s, struct node_id *peer)
{
  ...
  err =
    rdma_create_id (peer->rpc_ec, &(peer->rpc_conn.id), NULL, RDMA_PS_TCP);
  if (err != 0)
    {
      printf ("rdma_create_id %d in %s.\n", err, __func__);
      goto err_out;
  ...
  while (rdma_get_cm_event (peer->rpc_ec, &event) == 0)
      ...
      if (event_copy.event == RDMA_CM_EVENT_ADDR_RESOLVED)
	{
	  ...
	  err =
	    rdma_create_qp (event_copy.id, peer->rpc_conn.pd,
			    &peer->rpc_conn.qp_attr);
	  if (err != 0)
	    {
	      printf
		("Peer %d could not connect to peer %d. Current number of qp is  %d\n rdma_create_qp %d in %s %s.\n",
		 rpc_s->ptlmap.id, peer->ptlmap.id, rpc_s->num_qp, err,
...
//                                      printf("I was %d now I am %d minid %d %d\n", rpc_s->ptlmap.id, ((struct con_param *) event_copy.param.conn.private_data)->pm_sp.id, rpc_s->app_minid, ((struct con_param *) event_copy.param.conn.private_data)->type);
		  rpc_s->app_minid =
		    ((struct con_param *) event_copy.param.conn.
		     private_data)->type;
		  rpc_s->ptlmap.id =
		    ((struct con_param *) event_copy.param.conn.
		     private_data)->pm_sp.id;
...
//printf("id is %d, appminid is%d.\n", rpc_s->ptlmap.id, rpc_s->app_minid);
		...
		rpc_s->ptlmap.id =
		  *((int *) event_copy.param.conn.private_data);
	    }
...
int
sys_connect (struct rpc_server *rpc_s, struct node_id *peer)
{
  struct addrinfo *addr;
  struct rdma_cm_event *event = NULL;
  struct rdma_conn_param cm_params;
  ...
  err =
    rdma_create_id (peer->sys_ec, &(peer->sys_conn.id), NULL, RDMA_PS_TCP);
  if (err != 0)
    {
      printf ("rdma_create_id %d in %s.\n", err, __func__);
      goto err_out;
  ...
  while (rdma_get_cm_event (peer->sys_ec, &event) == 0)
      ...
      if (event_copy.event == RDMA_CM_EVENT_ADDR_RESOLVED)
	{
	  ...
	  err =
	    rdma_create_qp (event_copy.id, peer->sys_conn.pd,
			    &peer->sys_conn.qp_attr);
	  if (err != 0)
	    {
	      printf
		("Peer %d couldnot connect to peer %d. Current number of qp is  %d\n rdma_create_qp %d in %s %s.\n",
		 rpc_s->ptlmap.id, peer->ptlmap.id, rpc_s->num_qp, err,
		 __func__, strerror (errno));
	      goto err_out;
	    }
...
*/
struct msg_buf *
msg_buf_alloc (struct rpc_server *rpc_s, const struct node_id *peer, int num_rpcs)	//Done
{
  struct msg_buf *msg;
  size_t size;
...
void
rpc_add_service (enum cmd_type rpc_cmd, rpc_service rpc_func)	//Done
{
...
//Use poll 2
static int
__process_event (struct rpc_server *rpc_s, int timeout)	//Done
{
  struct node_id *peer;
...
//                              peer = rpc_s->peer_tab + which_peer[i];
	      err =
		ibv_get_cq_event (peer->rpc_conn.comp_channel, &ev_cq,
				  &ev_ctx);
	      if (err == -1 && errno == EAGAIN)
		{
...
int
rpc_process_event (struct rpc_server *rpc_s)	//Done
{
  int err;
...
int
rpc_process_event_with_timeout (struct rpc_server *rpc_s, int timeout)	//Done
{
  int err;
...
*/
int
rpc_send (struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg)	//Done
{
  if (peer->rpc_conn.f_connected != 1)
    {
...
inline static int
__send_direct (struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg, flag_t f_vec)	//Done
{
  if (peer->rpc_conn.f_connected != 1)
    {
      rpc_connect (rpc_s, peer);
...
int
rpc_send_direct (struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg)	//Done
{
  int err;
...
inline static int
__receive (struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg, flag_t f_vec)	//Done
{
  if (peer->rpc_conn.f_connected != 1)
    {
      rpc_connect (rpc_s, peer);
...
int
rpc_receive (struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg)	//Done
{
  int err;
...
int
rpc_receive_direct (struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg)	//Done
{
  if (peer->rpc_conn.f_connected != 1)
    {
      printf ("RPC channel has not been established  %d %d\n",
...
int
rpc_send_directv (struct rpc_server *rpc_s, struct node_id *peer,
		  struct msg_buf *msg)
{
  printf ("%s.\n", __func__);
  return 0;
...
//Not be used in InfiniBand version. Just keep an empty func here.
int
rpc_receivev (struct rpc_server *rpc_s, struct node_id *peer,
	      struct msg_buf *msg)
{
  printf ("%s.\n", __func__);
  return 0;
...
*/
int
rpc_read_config (int appid, struct sockaddr_in *address)	////done
{
  char *ip;
  char *port;
  FILE *f;
...
int
rpc_write_config (int appid, struct rpc_server *rpc_s)	////done
{
  FILE *f;
  int err;
  ...
  err =
    fprintf (f, "P2TNID=%s\nP2TPID=%d\n",
	     inet_ntoa (rpc_s->ptlmap.address.sin_addr),
...
void
rpc_report_md_usage (struct rpc_server *rpc_s)
{
...
void
rpc_print_connection_err (struct rpc_server *rpc_s, struct node_id *peer,
...
// %s .\n", inet_ntoa(peer->ptlmap.address.sin_addr));
  ...
  printf ("peer# %d (%s)  to peer# %d (%s:%d)\n", rpc_s->ptlmap.id,
	  inet_ntoa (rpc_s->ptlmap.address.sin_addr), peer->ptlmap.id,
	  inet_ntoa (peer->ptlmap.address.sin_addr));
  return;
}

void
rpc_mem_info_cache (struct node_id *peer, struct msg_buf *msg,
		    struct rpc_cmd *cmd)
{
...
void
rpc_mem_info_reset (struct node_id *peer, struct msg_buf *msg,
		    struct rpc_cmd *cmd)
{
  return;
}
...
void
rpc_cache_msg (struct msg_buf *msg, struct rpc_cmd *cmd)
{
  msg->id = cmd->wr_id;
  msg->mr = cmd->mr;
