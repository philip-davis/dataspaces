...
static struct app_info *
app_find (struct dart_server *ds, int appid)
{
  struct app_info *app;
...
static int
dsrpc_unregister (struct rpc_server *rpc_s, struct rpc_cmd *cmd)	//Done
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
  ds->f_s_unreg = 0;
  return 0;
...
static int
ds_remove_app (struct dart_server *ds, struct node_id *peer, int appid)
{
  struct msg_buf *msg;
...
static int
dsrpc_cn_unregister_app (struct rpc_server *rpc_s, struct rpc_cmd *cmd)
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
...
//      printf("I am peer %d removing app %d before I have %d cp\n", rpc_s->ptlmap.id, appid,ds->num_cp);
  ...
  list_for_each_entry_safe (temp_peer, t, &ds->rpc_s->peer_list,
...
static int
dsrpc_cn_unregister (struct rpc_server *rpc_s, struct rpc_cmd *cmd)
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
  struct hdr_register *hreg = (struct hdr_register *) (cmd->pad);
  struct msg_buf *msg;
...
static int
dsrpc_cn_unregister2 (struct rpc_server *rpc_s, struct rpc_cmd *cmd)
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
  struct hdr_register *hreg = (struct hdr_register *) (cmd->pad);
  struct msg_buf *msg;
  ...
  int i;
  int in_charge =
    (ds->num_cp / ds->num_sp) + (ds->rpc_s->ptlmap.id <
				 ds->num_cp % ds->num_sp);
  static int num_unreg = 0;
  if (ds->f_stop != 1)
    {
...
*/
static int
dsrpc_cn_register (struct rpc_server *rpc_s, struct hdr_register *hdr)	//Done
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
  struct app_info *app;
  struct node_id *peer;
...
static int
ds_disseminate2 (struct dart_server *ds, struct node_id *peer)	//Done
{
  struct msg_buf *msg;
  struct hdr_register *hreg;
  struct ptlid_map *pptlmap;
  ...
  list_for_each_entry (temp_peer, &ds->rpc_s->peer_list, struct node_id,
		       peer_entry)
  {
...
static int
announce_app_completion (struct rpc_server *rpc_s, struct msg_buf *msg)
{
...
static int
announce_cp_completion (struct rpc_server *rpc_s, struct msg_buf *msg)	//Done
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
  struct app_info *app;
//      struct node_id *peer;
...
//printf("In '%s()'.\n", __func__);
  for (i = 0; i < ds->num_sp; i++)
      ...
      if (temp_peer->ptlmap.id == 0)
	{
      ...
      if (temp_peer->ptlmap.address.sin_addr.s_addr ==
	  ds->rpc_s->ptlmap.address.sin_addr.s_addr
	  && temp_peer->ptlmap.address.sin_port ==
	  ds->rpc_s->ptlmap.address.sin_port)
	ds->self = temp_peer;
...
static int
ds_disseminate_app (struct dart_server *ds, struct node_id *peer, int appid)
{
  struct msg_buf *msg;
  struct hdr_register *hreg;
  struct ptlid_map *pptlmap;
...
static int
cs_disseminate_cp (struct dart_server *ds, struct node_id *peer)
{
  ...
  msg->cb = default_completion_with_data_callback;
  msg->size =
    sizeof (struct ptlid_map) * (ds->num_sp + ds->current_client_size -
				 app->app_num_peers);
  pptlmap = msg->msg_data = malloc (msg->size);
  if (!msg->msg_data)
    goto err_out_free;
...
static int
cp_disseminate_cs_completion (struct rpc_server *rpc_s, struct msg_buf *msg)
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
  struct app_info *app;
...
static int
dsrpc_cp_disseminate_cs (struct rpc_server *rpc_s, struct rpc_cmd *cmd)
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
...
static int
dsrpc_announce_app (struct rpc_server *rpc_s, struct rpc_cmd *cmd)
{
...
static int
dsrpc_announce_cp (struct rpc_server *rpc_s, struct rpc_cmd *cmd)	//Done
{
  struct dart_server *ds = ds_ref_from_rpc (rpc_s);
...
//        int connected = 0;
  ...
  while (ds->rpc_s->thread_alive
	 && (rdma_get_cm_event (ds->rpc_s->rpc_ec, &event) == 0))
    {
      /*      
         if(ds->num_cp>0){
         check_num_cp==0;
         ...
         else{tm_end = time(NULL);
         if((int)tm_end - (int)tm_st>10){
         ds->f_stop = 1;
         }
         }
         }    
...
//                                      list_add(&temp_peer->peer_entry, &ds->rpc_s->peer_list);
	  ...
	  if (err != 0)
		...
		("Peer %d couldnot connect to peer %d. Current number of qp is  %d\n rdma_create_qp %d in %s %s.\n",
		 ds->rpc_s->ptlmap.id, peer->ptlmap.id, ds->rpc_s->num_qp,
		 err, __func__, strerror (errno));
	      goto err_out;
	    }
	  ds->rpc_s->num_qp++;
...
int
ds_boot_master (struct dart_server *ds)	//Done
{
  struct rdma_cm_event *event = NULL;
  int connect_count = 0, err;
  int connected = 0;
  ...
  rc =
    pthread_create (&(ds->rpc_s->comm_thread), NULL, ds_master_listen,
		    (void *) ds);
  if (rc)
    {
...
*/
int
ds_boot_slave (struct dart_server *ds)	//Done
{
//      struct node_id *peer = ds_get_peer(ds, 0);
  struct rdma_conn_param cm_params;
  struct con_param conpara;
...
//        peer->ptlmap.id = 0;
  ...
  if (err < 0)
    goto err_out;
  if (peer->ptlmap.address.sin_addr.s_addr ==
      ds->rpc_s->ptlmap.address.sin_addr.s_addr
      && peer->ptlmap.address.sin_port == ds->rpc_s->ptlmap.address.sin_port)
    {
      ...
      printf
	("'%s()': WARNING! config file exists, but I am the master server\n",
	 __func__);
...
//      ds->rpc_s->peer_tab[1].ptlmap = ds->rpc_s->ptlmap;      //diff
  ...
  rc =
    pthread_create (&(ds->rpc_s->comm_thread), NULL, ds_listen, (void *) ds);
  if (rc)
    {
      printf ("ERROR; return code from pthread_create() is %d\n", rc);
      exit (-1);
...
/*
 ...
 printf("request from %d\n", conpara.pm_cp.id);
				...
				peer = rpc_server_find(ds->rpc_s, conpara.pm_cp.id);
 printf("after request from %d\n", conpara.pm_cp.id);
...
static int
ds_boot (struct dart_server *ds)	//Done
{
  struct stat st_buff;
...
*/
struct dart_server *
ds_alloc (int num_sp, int num_cp, void *dart_ref)
{
  ...
  int i, err;
  size =
    sizeof (struct dart_server) + (num_sp + num_cp) * sizeof (struct node_id);
  ds = calloc (1, size);
  if (!ds)
    goto err_out;
...
//      ds->cn_peers = ds->peer_tab + num_sp;
  ...
  //       INIT_LIST_HEAD(&ds->rpc_s->peer_list);


  cp_rank_cnt = num_sp;
  ds->rpc_s =
    rpc_server_init (0, NULL, 0, 10, ds->peer_size, ds, DART_SERVER);
  if (!ds->rpc_s)
    goto err_free_dsrv;
...
int
ds_process (struct dart_server *ds)
{
