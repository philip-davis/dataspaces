...
static int
dart_rdma_get (struct dart_rdma_tran *read_tran, struct dart_rdma_op *read_op)
{
  int err = -ENOMEM;
  ...
  read_op->wr.wr.rdma.remote_addr =
    read_tran->src.mr.addr + read_op->src_offset;
...
static int
dart_rdma_process_ibv_cq (struct dart_rdma_tran *read_tran)
{
...
int
dart_rdma_init (struct rpc_server *rpc_s)
{
  int err = -ENOMEM;
  if (drh)
    {
...
int
dart_rdma_register_mem (struct dart_rdma_mem_handle *mem_hndl,
			void *data, size_t bytes)
{
  ...
  mem_hndl->mr_ref = ibv_reg_mr (drh->rpc_s->global_pd, data, bytes,
...
int
dart_rdma_deregister_mem (struct dart_rdma_mem_handle *mem_hndl)
{
  int err = -ENOMEM;
...
int
dart_rdma_set_memregion_to_cmd (struct dart_rdma_mem_handle *mem_hndl,
				struct rpc_cmd *cmd)
{
  cmd->mr = mem_hndl->mr;
  return 0;
...
int
dart_rdma_get_memregion_from_cmd (struct dart_rdma_mem_handle *mem_hndl,
				  struct rpc_cmd *cmd)
{
  mem_hndl->mr = cmd->mr;
  return 0;
...
int
dart_rdma_create_read_tran (struct node_id *remote_peer,
...
int
dart_rdma_delete_read_tran (int tran_id)
{
...
int
dart_rdma_schedule_read (int tran_id, size_t src_offset, size_t dst_offset,
			 size_t bytes)
{
  int err = -ENOMEM;
  if (!drh)
...
int
dart_rdma_perform_reads (int tran_id)
{
...
int
dart_rdma_check_reads (int tran_id)
{
