...
*/
static int
barrier_broadcast (struct dart_client *dc)
{
  struct msg_buf *msg;
  struct node_id *peer;
  int i, err;
...
*/
static int
dcrpc_barrier (struct rpc_server *rpc_s, struct rpc_cmd *cmd)
{
...
static int
announce_cp_completion_all (struct rpc_server *rpc_s, struct msg_buf *msg)	//Done
{
  struct dart_client *dc = dc_ref_from_rpc (rpc_s);
  struct node_id *peer;
  struct ptlid_map *pm;
...
//                printf("HELLO Client %d peer %d:  %s %d \n",rpc_s->ptlmap.id, temp_peer->ptlmap.id, inet_ntoa(temp_peer->ptlmap.address.sin_addr),ntohs(temp_peer->ptlmap.address.sin_port));
	  ...
	  dc->rpc_s->ptlmap.address.sin_addr.s_addr
	  && temp_peer->ptlmap.address.sin_port ==
	  dc->rpc_s->ptlmap.address.sin_port)
	{
...
static int
dcrpc_announce_cp_all (struct rpc_server *rpc_s, struct rpc_cmd *cmd)	//Done
{
  struct dart_client *dc = dc_ref_from_rpc (rpc_s);
...
//        err = rpc_receive_direct(rpc_s, peer, msg);
  list_for_each_entry (temp_peer, &dc->rpc_s->peer_list, struct node_id,
		       peer_entry)
  {
...
static int
dcrpc_announce_cp (struct rpc_server *rpc_s, struct rpc_cmd *cmd)	//Done
{
  struct dart_client *dc = dc_ref_from_rpc (rpc_s);
  struct hdr_register *hreg = (struct hdr_register *) cmd->pad;
  struct node_id *peer;
...
//      peer = rpc_s->peer_tab;
  ...
  if (!msg)
    goto err_out;
  msg->size =
    sizeof (struct ptlid_map) * (hreg->num_cp + hreg->num_sp - dc->peer_size -
				 1);
  msg->msg_data = malloc (msg->size);
  if (!msg->msg_data)
    {
...
static int
dcrpc_unregister (struct rpc_server *rpc_s, struct rpc_cmd *cmd)	//Done
{
...
static int
dcrpc_unregister_cp (struct rpc_server *rpc_s, struct rpc_cmd *cmd)	//Done
{
  struct dart_client *dc = dc_ref_from_rpc (rpc_s);
  dc->num_unreg++;
  struct msg_buf *msg;
...
static int
dc_unregister (struct dart_client *dc)	//Done
{
  struct msg_buf *msg;
...
static int
dc_unregister2 (struct dart_client *dc)	//Done
{
  struct msg_buf *msg;
  struct hdr_register *hreg;
  struct node_id *peer;
...
dc_listen (void *client)
{
  ...
  while (dc->rpc_s->thread_alive
	 && (rdma_get_cm_event (dc->rpc_s->rpc_ec, &event) == 0))
    {
      ...
      if (event_copy.event == RDMA_CM_EVENT_CONNECT_REQUEST)
	{
	  ...
	  while (peer == NULL)
		...
		printf ("STUCK %d, waitting for %d %d\n",
			dc->rpc_s->ptlmap.id, conpara.pm_cp.id,
			dc->rpc_s->num_peers);
	      kk++;
	  ...
	  if (conpara.type == 0)
		...
		{
		  printf
		    ("Client %d: SYS connect request from %d, but I am already connected\n",
		     dc->rpc_s->ptlmap.id, peer->ptlmap.id);
		}
...
//                                      list_add(&temp_peer->peer_entry, &dc->rpc_s->peer_list);
	  ...
	  if (err != 0)
		...
		("Peer %d couldnot connect to peer %d. Current number of qp is  %d\n rdma_create_qp %d in %s %s.\n",
		 dc->rpc_s->ptlmap.id, peer->ptlmap.id, dc->rpc_s->num_qp,
...
static int
dc_disseminate (struct dart_client *dc)
{
  struct msg_buf *msg;
  struct hdr_register *hreg;
...
static int
dc_disseminate_dc (struct dart_client *dc)	//Done
{
  struct msg_buf *msg;
  struct hdr_register *hreg;
  struct node_id *peer;		//, *cpeer;
...
int
dc_boot_master (struct dart_client *dc)
{
  struct rdma_cm_event *event = NULL;
  int connect_count = 0, err;
  int connected = 0;
  ...
  rc =
    pthread_create (&(dc->rpc_s->comm_thread), NULL, dc_master_listen,
		    (void *) dc);
  if (rc)
    {
...
int
dc_boot_slave (struct dart_client *dc)	//Done
{
...
//      dc->rpc_s->peer_tab[1].ptlmap = dc->rpc_s->ptlmap;      //diff
  ...
  rc =
    pthread_create (&(dc->rpc_s->comm_thread), NULL, dc_listen, (void *) dc);
  if (rc)
    {
      printf ("ERROR; return code from pthread_create() is %d\n", rc);
      exit (-1);
...
static int
dc_boot (struct dart_client *dc, int appid)
{
  struct stat st_buff;
  char fil_lock[10];
...
static int
dc_boot2 (struct dart_client *dc, int appid)	//Done
{
...
*/
struct dart_client *
dc_alloc (int num_peers, int appid, void *dart_ref)
{
...
int
dc_process (struct dart_client *dc)
{
