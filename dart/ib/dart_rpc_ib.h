...
*/
static int
default_completion_callback (struct rpc_server *rpc_s, struct msg_buf *msg)
{
  free (msg);
  return 0;
}
...
static int
default_completion_with_data_callback (struct rpc_server *rpc_s,
				       struct msg_buf *msg)
{
  if (msg->msg_data)
    free (msg->msg_data);
...
struct rpc_server *rpc_server_init (int option, char *ip, int port,
				    int num_buff, int num_rpc_per_buff,
				    void *dart_ref,
...
void rpc_server_set_rpc_per_buff (struct rpc_server *rpc_s,
				  int num_rpc_per_buff);
...
int rpc_send (struct rpc_server *rpc_s, struct node_id *peer,
	      struct msg_buf *msg);
int rpc_send_direct (struct rpc_server *rpc_s, struct node_id *peer,
		     struct msg_buf *msg);
...
int rpc_receive_direct (struct rpc_server *rpc_s, struct node_id *peer,
			struct msg_buf *msg);
...
void rpc_print_connection_err (struct rpc_server *rpc_s, struct node_id *peer,
