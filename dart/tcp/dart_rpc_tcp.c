...
static int socket_send_bytes(int sockfd, char *buffer, uint64_t size) {
    while (size > 0) {
        ...
        if (n < 0) {
            uloga("[%s]: send bytes through socket failed!\n", __func__);
            goto err_out;
        }
        buffer += n;
        size -= n;
...
static int socket_recv_bytes(int sockfd, char *buffer, uint64_t size, int f_blocking) {
    ...
    while (size > 0) {
        ...
        if (n < 0) {
            uloga("[%s]: receive bytes through socket failed!\n", __func__);
            goto err_out;
        }
        if (n == 0) {
            uloga("[%s]: connection has already closed, skip!\n", __func__);
            goto err_out;
        }
        buffer += n;
        size -= n;
...
static int socket_recv_rpc_cmd(int sockfd, struct rpc_cmd *cmd) {
    ...
    if (ret < 0) {
        uloga("[%s]: receive RPC command through socket failed!\n", __func__);
        goto err_out;
    }
    if (ret == 1) {
...
int rpc_recv_connection_info(int sockfd, struct connection_info *info) {
    if (socket_recv_bytes(sockfd, (char *)info, (uint64_t)sizeof(*info), 1) < 0) {
        uloga("[%s]: recv connection info failed!\n", __func__);
        goto err_out;
    }
    return 0;
...
static int rpc_cb_request_posted(struct rpc_server *rpc_s, struct rpc_request *request) {
    if (request->msg != NULL && request->msg->cb != NULL) {
        if ((*request->msg->cb)(rpc_s, request->msg) < 0) {
            uloga("[%s]: call message callback function failed!\n", __func__);
            goto err_out;
        }
    }
    free(request);
...
struct rpc_server* rpc_server_init(const char *interface, int app_num_peers, void *dart_ref, enum rpc_component cmp_type) {
    ...
    if (rpc_s == NULL) {
        uloga("[%s]: allocate RPC server failed!\n", __func__);
        goto err_out;
    }
    ...
    if (rpc_server_init_socket(rpc_s) < 0) {
        uloga("[%s]: initialize socket for RPC server failed!\n", __func__);
        goto err_out;
    }
...
int rpc_write_config(struct rpc_server *rpc_s, const char *filename) {
    ...
    if(f == NULL) {
        uloga("[%s]: open config file failed!\n", __func__);
        goto err_out;
    }
    ...
    if (fprintf(f, "P2TNID=%s\nP2TPID=%d\n", ip, port) < 0) {
        uloga("[%s]: write config file failed!\n", __func__);
        goto err_out;
    }
...
int rpc_read_config(struct sockaddr_in *address, const char *filename) {
    ...
    if (ip == NULL || port == NULL) {
        ...
        if (fscanf(f, "P2TNID=%32s\nP2TPID=%32s\n", ip_buff, port_buff) != 2) {
            uloga("[%s]: read config file failed!\n", __func__);
            goto err_out;
        }
        fclose(f);
        f = NULL;
...
int rpc_send(struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg) {
    ...
    if (request == NULL) {
        uloga("[%s]: allocate request failed!\n", __func__);
        goto err_out;
    }
    ...
    if (peer_process_send_list(rpc_s, peer) < 0) {
        uloga("[%s]: process send list for peer %d failed!\n", __func__, peer->ptlmap.id);
        goto err_out;
    }
    return 0;
...
int rpc_send_direct(struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg) {
    if (!peer->f_connected) {
        uloga("[%s]: cannot send to an unconnected peer directly!\n", __func__);
        goto err_out;
    }
...
int rpc_receive_direct(struct rpc_server *rpc_s, struct node_id *peer, struct msg_buf *msg) {
    if (!peer->f_connected) {
        uloga("[%s]: cannot receive from an unconnected peer directly!\n", __func__);
        goto err_out;
    }
